...
async def process_scene(session, i, scene):
    ...
    try:
        ...
        # ----------------------------------------------------------------------
        # WATERMARK UPDATE LOGIC: (x=w-tw-40:y=40 places it in the Top-Right)
        # ----------------------------------------------------------------------
        if is_valid_video:
    ...
    async with aiohttp.ClientSession() as session:
        ...
        if os.path.exists(bgm_path):
            bgm_cmd = [
                ...
                '-filter_complex', '[0:a]volume=1.0[voice];[1:a]volume=0.14[bgm];[voice][bgm]amix=inputs=2:duration=first:dropout_transition=0[aout_mix];[aout_mix]volume=2.0[aout]',
                '-map', '0:v', '-map', '[aout]',
